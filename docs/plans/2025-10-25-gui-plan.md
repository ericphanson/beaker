...
- JSON serialization overhead for all Rust ↔ Frontend communication
- Image data requires base64 encoding (overhead for image-heavy apps like Beaker)
- "In applications with many messages containing lots of data, this could be a performance hit"
...
| **IPC overhead** | "JSON serialization for images?" | True concern - for image-heavy workflows, could be bottleneck |

**For Beaker Specifically:**

⚠️ **Image-Heavy Workflow Concern:**
- Beaker processes images extensively (detection, cutout, quality)
- Every image needs to cross IPC boundary (Rust → Frontend for display)
- Options:
  1. Base64 encode (bloated, slow for large images)
  2. Write to disk, send file path (extra I/O)
  3. Keep UI minimal, only show results (less interactive)

✅ **But Aesthetics Would Be Better:**
...
If you're comfortable with modern web development (React/Svelte + TypeScript), Tauri is a great choice despite the IPC overhead. If you want to avoid npm/webpack/babel ecosystem entirely, stick with native Rust solutions (egui, cacao, etc.).
