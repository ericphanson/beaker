...
class OnnxOptimizer:
    ...
    def common_opt(self, return_onnx=False):
        ...
        onnx_graph = fold_constants(
            gs.export_onnx(self.graph), allow_onnxruntime_shape_inference=False
        )
    ...
    def resize_fix(self):
        ...
        for node in self.graph.nodes:
            if node.op == "Resize" and len(node.inputs) == 3:
                ...
                shape_hw_out = gs.Variable(
                    name=name + "shape_hw_out", dtype=np.int64, shape=[4]
                )
                shape_hw = gs.Node(
                    op="Shape",
                    name=name + "shape_hw",
                ...
                const_zero = gs.Constant(
                    name=name + "const_zero", values=np.array([0], dtype=np.int64)
                )
                const_two = gs.Constant(
                    name=name + "const_two", values=np.array([2], dtype=np.int64)
                )
                const_four = gs.Constant(
                    name=name + "const_four", values=np.array([4], dtype=np.int64)
                )

                slice_hw_out = gs.Variable(
                    name=name + "slice_hw_out", dtype=np.int64, shape=[2]
                )
                slice_hw = gs.Node(
                    op="Slice",
                    name=name + "slice_hw",
                ...
                shape_bc_out = gs.Variable(
                    name=name + "shape_bc_out", dtype=np.int64, shape=[2]
                )
                shape_bc = gs.Node(
                    op="Shape",
                    name=name + "shape_bc",
                    inputs=[div_node.outputs[0]],
                    outputs=[shape_bc_out],
                )

                slice_bc_out = gs.Variable(
                    name=name + "slice_bc_out", dtype=np.int64, shape=[2]
                )
                slice_bc = gs.Node(
                    op="Slice",
                    name=name + "slice_bc",
                ...
                concat_bchw_out = gs.Variable(
                    name=name + "concat_bchw_out", dtype=np.int64, shape=[4]
                )
                concat_bchw = gs.Node(
                    op="Concat",
                ...
                self.graph.nodes.extend(
                    [shape_hw, slice_hw, shape_bc, slice_bc, concat_bchw, resize_bchw]
                )
        ...
        for node in self.graph.nodes:
            if node.op == "InstanceNormalization":
                ...
                sub_node = gs.Node(
                    ...
                    name=name + "sub_node",
                    attrs={},
                    inputs=[input_tensor, mean_out],
                    outputs=[sub_out],
                )
                pow_out = gs.Variable(name=name + "pow_out")
                pow_const = gs.Constant(
                    name=name + "pow_const", values=np.array([2.0], dtype=np.float32)
                )
                pow_node = gs.Node(
                    op="Pow",
                    name=name + "pow_node",
                    attrs={},
                    inputs=[sub_out, pow_const],
                    outputs=[pow_out],
                )
    ...
    def insert_groupnorm_plugin(self):
        ...
        for node in self.graph.nodes:
            if (
                ...
                and len(node.o().o().o().o().o().o().o().o().inputs[1].values.shape)
                == 3
            ):
                ...
                constantBeta = gs.Constant(
                    "groupNormBeta-" + str(nGroupNormPlugin),
                ...
                groupNormV = gs.Variable(
                    "GroupNormV-" + str(nGroupNormPlugin),
                    np.dtype(np.float16),
                    inputTensor.shape,
                )
                groupNormN = gs.Node(
    ...
    def insert_layernorm_plugin(self):
        ...
        for node in self.graph.nodes:
            if (
                ...
                constantBeta = gs.Constant(
                    "LayerNormBeta-" + str(nLayerNormPlugin),
                    np.ascontiguousarray(beta.reshape(-1)),
                )
    ...
    def fuse_kv(self, node_k, node_v, fused_kv_idx, heads, num_dynamic=0):
        ...
        weights_kv = np.dstack(
            [weights_k.reshape(C, H, D), weights_v.reshape(C, H, D)]
        ).reshape(C, 2 * H * D)
        ...
        # Create tensor
        constant_weights_kv = gs.Constant(
            "Weights_KV_{}".format(fused_kv_idx), np.ascontiguousarray(weights_kv)
        )
    ...
    def insert_fmhca(
        ...
        shape = gs.Constant(
            ...
            np.ascontiguousarray(
                np.array([0, 0, heads, 2, dims_per_head], dtype=np.int64)
            ),
        )

        # Reshape output tensor
        output_reshape = gs.Variable(
            "ReshapeKV_{}".format(mhca_idx), np.dtype(np.float16), None
        )
        # Create fMHA plugin
        reshape = gs.Node(
            op="Reshape",
            name="Reshape_{}".format(mhca_idx),
            inputs=[output_kv, shape],
            outputs=[output_reshape],
        )
        # Insert node
        ...
        if num_dynamic > 0:
            reshape2_input1_out = gs.Variable(
                "Reshape2_fmhca{}_out".format(mhca_idx), np.dtype(np.int64), None
            )
    ...
    def fuse_qkv(self, node_q, node_k, node_v, fused_qkv_idx, heads, num_dynamic=0):
        ...
        weights_qkv = np.dstack(
            [
                weights_q.reshape(C, H, D),
                weights_k.reshape(C, H, D),
                weights_v.reshape(C, H, D),
            ]
        ).reshape(C, 3 * H * D)
        ...
        output_tensor_k = node_k.outputs[0]
        # Concat and interleave weights such that the output of fused QKV GEMM has [b, s, h, 3, d] shape
        constant_weights_qkv = gs.Constant(
            "Weights_QKV_{}".format(fused_qkv_idx), np.ascontiguousarray(weights_qkv)
        )
    ...
    def insert_fmha(self, node_qkv, final_tranpose, mha_idx, heads, num_dynamic=0):
        ...
        shape = gs.Constant(
            ...
            np.ascontiguousarray(
                np.array([0, 0, heads, 3, dims_per_head], dtype=np.int64)
            ),
        )

        # Reshape output tensor
        output_shape = gs.Variable(
            "ReshapeQKV_{}".format(mha_idx), np.dtype(np.float16), None
        )
        # Create fMHA plugin
        reshape = gs.Node(
            op="Reshape",
            name="Reshape_{}".format(mha_idx),
            inputs=[output_qkv, shape],
        ...
        if num_dynamic > 0:
            reshape2_input1_out = gs.Variable(
                "Reshape2_{}_out".format(mha_idx), np.dtype(np.int64), None
            )
            reshape2_input1_shape = gs.Node(
                "Shape",
                "Reshape2_{}_shape".format(mha_idx),
                inputs=[node_qkv.inputs[0]],
                outputs=[reshape2_input1_out],
            )
            self.graph.nodes.append(reshape2_input1_shape)
            final_tranpose.o().inputs[1] = reshape2_input1_out
        ...
        for idx, _ in enumerate(nodes):
            ...
            if detected:
                ...
                if sm == 75 and node_q.inputs[1].shape[1] // heads == 160:
                    continue
                # Fuse K and V GEMMS
                node_kv = self.fuse_kv(
                    node_k, node_v, mhca_index, heads, num_dynamic_kv
                )
                # Insert fMHCA plugin
                self.insert_fmhca(
                    node_q, node_kv, final_tranpose, mhca_index, heads, num_dynamic_q
                )
                return True
        return False
    ...
    def fuse_qkv_insert_fmha(self, heads, mha_index):
        ...
        for idx, _ in enumerate(nodes):
            ...
            if detected:
                ...
                node_qkv = self.fuse_qkv(
                    node_q, node_k, node_v, mha_index, heads, num_dynamic_kv
                )
                # Insert fMHA plugin
                self.insert_fmha(
                    node_qkv, final_tranpose, mha_index, heads, num_dynamic_kv
                )
                return True
        return False
