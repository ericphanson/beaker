...
def parser_args():
    ...
    parser.add_argument(
        "--coco_path", type=str, default="data/coco", help="coco dataset path"
    )
    parser.add_argument("--device", default=0, type=int)
    parser.add_argument(
        "--run_benchmark",
        action="store_true",
        help="repeat the inference to benchmark the latency",
    )
    parser.add_argument(
        "--disable_eval", action="store_true", help="disable evaluation"
    )
...
def main(args):
    ...
    if args.run_benchmark:
        repeats = 10
        print(
            "Inference for each image will be repeated 10 times to obtain "
