...
use beaker_stamp::{write_cfg_stamp, Stamp};

/// Generate a deterministic configuration hash for detection using proc macro
///
/// This function now uses the Stamp trait implementation generated by the derive macro.
/// The derive macro automatically includes all fields marked with #[stamp] in DetectionConfig.
pub fn generate_detection_config_hash(config: &DetectionConfig) -> String {
    let full_hash = config.stamp_hash();
    // Extract just the hex part (remove "sha256:" prefix) and truncate to 16 chars for consistency
    full_hash.trim_start_matches("sha256:")[..16].to_string()
}

/// Generate a deterministic configuration hash for cutout processing using proc macro
///
/// This function now uses the Stamp trait implementation generated by the derive macro.
/// The derive macro automatically includes all fields marked with #[stamp] in CutoutConfig.
...
pub fn generate_detection_stamps(
    ...
    let config_stamp = write_cfg_stamp("detect", config)
        .map_err(|e| anyhow!("Failed to write detection config stamp: {}", e))?;
...
pub fn generate_cutout_stamps(
    ...
    // Generate config stamp using the new proc macro approach
    let config_stamp = write_cfg_stamp("cutout", config)
        .map_err(|e| anyhow!("Failed to write cutout config stamp: {}", e))?;
