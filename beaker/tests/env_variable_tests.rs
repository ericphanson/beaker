...
fn test_head_model_path_override() {
    ...
    let output = Command::new(&beaker_binary)
        .args(["head", test_image.to_str().unwrap(), "--confidence", "0.5", "--metadata"])
...
fn test_cutout_model_path_override() {
    ...
    let beaker_binary = Path::new(env!("CARGO_MANIFEST_DIR")).join("target/debug/beaker");
    let output = Command::new(&beaker_binary)
        .args(["cutout", test_image.to_str().unwrap()])
        .env("BEAKER_CUTOUT_MODEL_PATH", "/non/existent/cutout_model.onnx")
        .current_dir(env!("CARGO_MANIFEST_DIR"))
        .output()
        .expect("Failed to execute beaker command");
...
fn test_version_command_with_env_vars() {
    ...
    assert!(stdout.contains("Environment Variables:"), "Should show environment variables");
    assert!(stdout.contains("BEAKER_DEBUG: true"), "Should show BEAKER_DEBUG");
...
fn test_metadata_captures_env_vars() {
    ...
    assert!(metadata_content.contains("[head.execution.beaker_env_vars]"), 
           "Should contain environment variables section");
    assert!(metadata_content.contains("BEAKER_DEBUG = \"test_value\""), 
           "Should contain BEAKER_DEBUG variable");
    assert!(metadata_content.contains("RUST_LOG = \"debug\""), 
