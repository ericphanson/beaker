...
def analyze_timing_data(timing_data, operation_name):
    ...
    print(f"\n=== SUMMARY ===")
    print(f"Total model processing: {total_model_time:.1f}ms ({100*total_model_time/total_time:.1f}%)")
    print(f"Total file I/O:         {total_io_time:.1f}ms ({100*total_io_time/total_time:.1f}%)")
    ...
    if total_time > 0:
        ...
        print(f"(Eliminates intermediate file writes between pipeline steps)")
    ...
    return {
        ...
        'pipeline_savings_pct': 100 * total_io_write_time / total_time if total_time > 0 else 0
    }
...
def main():
    ...
    if not beaker_path:
        print("Error: Could not find beaker binary. Please build first with 'cargo build --release'")
        return 1
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if combined_timing:
            pipeline_analysis = analyze_timing_data(combined_timing, "Combined Pipeline")
            
            print("\n" + "="*60)
            print("PIPELINE FEASIBILITY ASSESSMENT")
            print("="*60)
            print(f"Current file I/O overhead: {pipeline_analysis['io_percentage']:.1f}% of total time")
            print(f"Potential pipeline improvement: {pipeline_analysis['pipeline_savings_pct']:.1f}%")
            
            if pipeline_analysis['pipeline_savings_pct'] < 2.0:
                print("\n⚠️  CONCLUSION: Limited performance benefit")
                print("   Pipeline value should focus on ergonomics and workflow simplification")
            else:
