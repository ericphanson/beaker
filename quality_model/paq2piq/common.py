...
def blend_output(
    input_image, output, vmin=0, vmax=100, alpha=0.8, resample=Image.BILINEAR
):
...
def render_output(input_image, output, vmin=0, vmax=100, fig=None):
    ...
    mat = axes[2].imshow(
        output["local_scores"], cmap="gray", extent=extent, vmin=vmin, vmax=vmax
    )
