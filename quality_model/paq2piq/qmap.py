...
class QualityMap:
    ...
    @property
    def pil_image(self):
        return Image.fromarray((255 * image2np(self.img.px)).astype(np.uint8))
    ...
    def plot(self, vmin=0, vmax=100):
        ...
        self.img.show(
            axes[1], title=f"Predicted: {self.global_score:.2f}"
        )  # title prediction
        ...
        axes[1].imshow(
            self.mat, alpha=0.8, cmap="magma", extent=extent, interpolation="bilinear"
        )
        axes[2].imshow(self.mat, cmap="gray", extent=extent, vmin=vmin, vmax=vmax)
    ...
    def blend(self, mos_range=(0, 100), alpha=0.8, resample=PIL_Image.BILINEAR):
        ...
        if mos_range is not None:
            qmap_matrix = 100 * stretch(
                np.array(qmap_matrix), mos_range[0], mos_range[1]
            )
